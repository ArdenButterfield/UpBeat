...
SquareWaveSynth::SquareWaveSynth()
{
}

void SquareWaveSynth::prepareToPlay (double newSampleRate)
{
    sampleRate = newSampleRate;
    amplitudeDecrementPerSample = 1.0f / (decaySeconds * static_cast<float>(sampleRate));
}

void SquareWaveSynth::noteOn (int midiNote)
{
...
void SquareWaveSynth::renderNextBlock (juce::AudioBuffer<float>& buffer, int startSample, int numSamples)
{
    for (auto& voice : voices)
    {
        ...
        {
            auto value = (voice.phase < 0.5 ? 1.0f : -1.0f) * voice.amplitude * outputGain;
...
double SquareWaveSynth::midiNoteToFrequency (int midiNote)
{
